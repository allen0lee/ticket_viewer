...
class AuthenticationHelperTest < MiniTest::Unit::TestCase
  def test_valid_response_for_tickets
    ...
    assert_equal true, AuthenticationHelper.make_req_to_api(url).key?("tickets")
  end

  def test_valid_response_for_single_ticket_details
    url = "https://alanli.zendesk.com/api/v2/tickets/1.json"
    assert_equal true, AuthenticationHelper.make_req_to_api(url).key?("ticket")
  end

  def test_valid_response_for_requester_name
    url = "https://alanli.zendesk.com/api/v2/users/902093503063.json"
    assert_equal true, AuthenticationHelper.make_req_to_api(url).key?("user")
  end
  ...
  def test_couldnot_authenticate
    ...
    assert_equal true, AuthenticationHelper.make_req_to_api(url).key?("error")
  end
end